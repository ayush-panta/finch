...
var testNativeCredHelper = func(o *option.Option, installed bool) {
	ginkgo.Describe("Native Credential Helper", func() {
		...
		ginkgo.It("should manage daemon lifecycle", func() {
			...
			fmt.Printf("[TEST] Reading PID file: %s\n", pidFile)
			...
			fmt.Printf("[TEST] Daemon process %s is running\n", pid)

			fmt.Printf("[TEST] Stopping VM...\n")
			command.Run(o, virtualMachineRootCmd, "stop")
			fmt.Printf("[TEST] VM stopped, waiting for daemon to terminate...\n")
			...
			gomega.Eventually(func() error {
				...
				if err != nil {
					fmt.Printf("[TEST] Daemon process %s is dead\n", pid)
				} else {
					fmt.Printf("[TEST] Daemon process %s still alive, waiting...\n", pid)
				}
				return err
			}, 5*time.Second, 100*time.Millisecond).Should(gomega.HaveOccurred(), "Daemon process should be stopped after VM stop")

			// Verify PID file is removed
			fmt.Printf("[TEST] Checking if PID file is removed...\n")
			_, err = os.Stat(pidFile)
			gomega.Expect(os.IsNotExist(err)).To(gomega.BeTrue(), "PID file should be removed")
			fmt.Printf("[TEST] PID file removed successfully\n")

			// Verify no orphaned credserver processes
			fmt.Printf("[TEST] Checking for orphaned credserver processes...\n")
			// #nosec G204 -- checking for credserver processes
			psCmd := exec.Command("pgrep", "-f", "credserver")
			output, err := psCmd.Output()
			if err == nil && len(strings.TrimSpace(string(output))) > 0 {
				fmt.Printf("[TEST] FAIL: Found orphaned processes: %s\n", string(output))
				ginkgo.Fail(fmt.Sprintf("Found orphaned credserver processes: %s", string(output)))
			} else {
				fmt.Printf("[TEST] No orphaned processes found\n")
			}
		})
	})
}
