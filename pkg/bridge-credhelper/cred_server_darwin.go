...
func StartCredentialServer(finchRootPath string) error {
	...
	if isRunning(pidFile) {
		fmt.Fprintf(os.Stderr, "[CREDS] Daemon already running\n")
		return nil
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[CREDS] Warning: failed to write PID file: %v\n", err)
	}
...
func StopCredentialServer() {
	fmt.Fprintf(os.Stderr, "[CREDS] Stopping daemon\n")
	...
	fmt.Fprintf(os.Stderr, "[CREDS] No daemon PID file found\n")
}
...
func killDaemon(pidFile string) bool {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[CREDS] Failed to kill daemon PID %d: %v\n", pid, err)
		return false
	}
	
	// Remove PID file
	os.Remove(pidFile)
	fmt.Fprintf(os.Stderr, "[CREDS] Daemon stopped (PID %d)\n", pid)
	return true
}
